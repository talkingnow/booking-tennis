...
| 2026-05-15 | 로그인 흐름 안정화 v2 (M1~M7) — BUG-1~11 픽스: `applyBootAutoLoginPolicy` 헬퍼 신규, App 부팅·Account 토글·Race/Quick 진입 모두 bootAutoLogin 정책 일관 적용 (OFF 시 keepAlive·자동 doLogin·hydrate 재호출·meta 잔영 모두 차단), LoginBadge OFF idle 분기 "🔒 수동 로그인 필요", authStore expired+no_account → 'no_account' / forget·doLogout meta 정리, tests/auth_audit.test.ts 신규 13건 + BUG-7 회귀 1건. 132/0 vitest, qa MCP 9 시나리오 PASS | 02_plan_login_audit_v2.md / 03_build_log_login_audit_v2.md / 06_qa_report_login_audit_v2.md / 커밋 f220083 |
| 2026-05-18 | Bundle-B 통합 (M1~M11, R1·R3·R4) — R1 피드백 채널 (api/feedback.ts Discord/Slack webhook + /feedback UI + redaction 클라이언트), R3 파주 2슬롯 묶음 (SitePolicy.maxConsecutiveSlots gy=1/pj=2, SlotPicker 인접 highlight, Quick 다중 선택, Race PriorityEntry hour→hours[] 마이그레이션 + fire cascade 갱신), R4 즉시예약 날짜 제한 (SitePolicy.advanceMinDays + getDateBounds KST 헬퍼 + Quick/Race date input min/max). 신규 30 tests, 166/0 vitest PASS, qa MCP L1~L12 PASS. R2 트래픽 audit 은 _workspace/07_traffic_audit.md 만 산출, 코드 변경 별도 PR 권고 | 02_plan_bundle_b.md / 03_build_log_bundle_b.md / 06_qa_report_bundle_b.md / 04_user_action_bundle_b.md (FEEDBACK_WEBHOOK_URL 설정 가이드) |
